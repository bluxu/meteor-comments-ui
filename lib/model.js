Comments = (function () {
  ...
  Meteor.methods({
    ...
    'comments/remove': function (documentId) {
      ...
      // Hacky way of removing from own profile
      collection.remove({ _id: documentId, referenceId: this.userId });
    },
    ...
    'comments/count': function (referenceId) {
      var nonVerifiedUserIds = Meteor.users.find({'emails.verified': false}, {fields : {_id : 1}}).fetch().map(
        function(o) {
          return o._id;
        }
      );
      return collection.find({$and: [{ referenceId: referenceId }, { userId: { $nin: nonVerifiedUserIds } }]}).count();
    },
    'comments/countAll': function() {
      var nonVerifiedUserIds = Meteor.users.find({'emails.verified': false}, {fields : {_id : 1}}).fetch().map(
        function(o) {
          return o._id;
        }
      );
      return collection.find({ userId: { $nin: nonVerifiedUserIds } }).count();
    }
  });


  if (Meteor.isServer) {
    // Thanks to http://stackoverflow.com/questions/18744665/how-to-get-a-published-collections-total-count-regardless-of-a-specified-limit
    // and the Meteor docs
    // To do - maybe migrate to https://atmospherejs.com/tmeasday/publish-counts
    // To do - is this strictly necessary, or can't we just use the methods above?
    Meteor.publish('comments/count', function(userId) {
      var self = this;
      check(userId, String);
      var count = 0;
      var initializing = true;
      var handle = collection.find({userId: userId}).observeChanges({
        added: function (id) {
          count++;
          if (!initializing)
            self.changed('comments/count', userId, {count: count});
        },
        removed: function (id) {
          count--;
          self.changed('comments/count', userId, {count: count});
      ...
      initializing = false;
      self.added('comments/count', userId, {count: count});
      self.ready();
      self.onStop(function () {
        handle.stop();
      });
    });
    Meteor.publishComposite('comments/all', function(limit) {
      // In principle this could be made reactive
      // That would probably require a rewrite of the publishComposite package
      // along the lines of http://matthewfieger.com/posts/me/2014/07/28/multiple-reactive-joins-in-meteor.html
      // or a denormalization
      check(limit, Number);
      var nonVerifiedUserIds = Meteor.users.find({'emails.verified': false}, {fields : {_id : 1}}).fetch().map(
        function(o) {
          return o._id;
      ...
      return {
        find: function () {
          return collection.find({ userId: { $nin: nonVerifiedUserIds } }, { limit: limit, sort: { createdAt: -1 } });
        },
        children: [{
          find: function (comment) {
            var userIds = _.difference(getUserIdsByComment(comment), nonVerifiedUserIds);
    ...
    Meteor.publishComposite('comments/reference', function (id, limit) {
      ...
      return {
        find: function () {
          return collection.find({$and: [{ referenceId: id }, { userId: { $nin: nonVerifiedUserIds } }]}, { limit: limit, sort: { createdAt: -1 } });
        },
        children: [{
          find: function (comment) {
            var userIds = _.difference(getUserIdsByComment(comment), nonVerifiedUserIds);
  ...
  return {
    ...
    addForUserId: function (userId, content) {
      // Server side only
      if (Meteor.isServer) {
        collection.insert (
          { referenceId: userId, content: content, userId: userId, createdAt: (new Date()), likes: [], replies: [] }
        );
      }
    },
